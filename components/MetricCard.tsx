...
const statusConfig = {
  ...
  pendente: { icon: Clock, color: "text-gray-400", bg: "bg-gray-50 border-gray-200", bar: "bg-gray-300", label: "PENDENTE" },
};
...
export function MetricCard({ metric }: { metric: MetricResult }) {
  ...
  const Icon = cfg.icon;
  const pct = metric.status === "pendente" ? 0 : Math.round((metric.score / metric.maxScore) * 100);
      ...
      <button className="w-full flex items-center justify-between gap-3" onClick={() => setOpen(!open)}>
        <div className="flex items-center gap-3 flex-1 min-w-0">
          ...
          <div className="flex items-center gap-1.5 flex-1 min-w-0">
            ...
            {metric.limited && (
              <AlertTriangle className="shrink-0 w-3.5 h-3.5 text-amber-400" aria-label="Dados parciais" />
            )}
          </div>
        </div>
